...
class YoloxObjectDetectionTiny(nn.Module):
    def __init__(self, num_classes):
        """ 這裡主要提供的是模型大小為Nano的版本
        Args:
            num_classes: 分類類別數
        """
        super(YoloxObjectDetectionTiny, self).__init__()
        ...
        self.C3_p3 = CSPLayer(int(2 * in_channels[0] * width), int(in_channels[0] * width), round(3 * depth),
                              False, act='silu')
        self.bu_conv2 = BaseConv(int(in_channels[0] * width), int(in_channels[0] * width), 3, 2, act='silu')
        self.C3_n3 = CSPLayer(int(2 * in_channels[0] * width), int(in_channels[1] * width), round(3 * depth),
                              False, act='silu')
        self.bu_conv1 = BaseConv(int(in_channels[1] * width), int(in_channels[1] * width), 3, 2, act='silu')
        ...
        for in_channel in in_channels:
            ...
            self.cls_convs.append(nn.Sequential(*[
                BaseConv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act='silu'),
                BaseConv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act='silu')
            ]))
...
def main():
    ...
    model = YoloxObjectDetectionTiny(num_classes=args.num_classes)
    ...
    with torch.no_grad():
        ...
        torch.onnx.export(model_script, images, 'YoloxObjectDetectionTiny.onnx', input_names=input_names,
