...
class BaseConv(nn.Module):
    def __init__(self, in_channels, out_channels, ksize, stride, bias=False, groups=1, act='silu'):
...
class Focus(nn.Module):
    def __init__(self, in_channels, out_channels, ksize=1, stride=1, act='silu'):
        ...
        self.conv = BaseConv(in_channels * 4, out_channels, ksize=ksize, stride=stride, act=act)
...
class CSPDarknet(nn.Module):
    def __init__(self):
        ...
        dep_mul, wid_mul = 0.33, 0.25
        base_channel = int(wid_mul * 64)
        base_depth = max(round(dep_mul * 3), 1)
        self.stem = Focus(3, base_channel, ksize=3, act='silu')
        self.dark2 = nn.Sequential(DWConv(base_channel, base_channel * 2, 3, 2, act='silu'),
                                   CSPLayer(base_channel * 2, base_channel * 2, n=base_depth, act='silu'))
        self.dark3 = nn.Sequential(DWConv(base_channel * 2, base_channel * 4, 3, 2, act='silu'),
                                   CSPLayer(base_channel * 4, base_channel * 4, n=base_depth * 3, act='silu'))
        self.dark4 = nn.Sequential(DWConv(base_channel * 4, base_channel * 8, 3, 2, act='silu'),
                                   CSPLayer(base_channel * 8, base_channel * 8, n=base_depth * 3, act='silu'))
        self.dark5 = nn.Sequential(DWConv(base_channel * 8, base_channel * 16, 3, 2, act='silu'),
    ...
    def forward(self, x):
        ...
        d4 = self.dark4(d3)
        d5 = self.dark5(d4)
        # TODO 檢查這裡怎麼不會有Sequence Construct錯誤
        outputs = [d3, d4, d5]
        return outputs


class YoloxObjectDetectionNano(nn.Module):
    def __init__(self, num_classes):
        """ 這裡主要提供的是模型大小為Nano的版本
        Args:
            num_classes: 分類類別數
        """
        super(YoloxObjectDetectionNano, self).__init__()
        """ 提供對應到正常訓練下的配置，本檔案提供的是模型大小為Nano的類別
        backbone_cfg = {
            'type': 'YOLOPAFPN',
            'depth': 0.33,
            'width': 0.25,
        ...
        self.lateral_conv0 = BaseConv(int(in_channels[2] * width), int(in_channels[1] * width), 1, 1, act='silu')
        self.C3_p4 = CSPLayer(int(2 * in_channels[1] * width), int(in_channels[1] * width), round(3 * depth),
                              False, act='silu')
        self.reduce_conv1 = BaseConv(int(in_channels[1] * width), int(in_channels[0] * width), 1, 1, act='silu')
        self.C3_p3 = CSPLayer(int(2 * in_channels[0] * width), int(in_channels[0] * width), round(3 * depth),
                              False, act='silu')
        self.bu_conv2 = DWConv(int(in_channels[0] * width), int(in_channels[0] * width), 3, 2, act='silu')
        self.C3_n3 = CSPLayer(int(2 * in_channels[0] * width), int(in_channels[1] * width), round(3 * depth),
                              False, act='silu')
        self.bu_conv1 = DWConv(int(in_channels[1] * width), int(in_channels[1] * width), 3, 2, act='silu')
        ...
        for in_channel in in_channels:
            self.stems.append(BaseConv(in_channels=int(in_channel * width), out_channels=int(256 * width), ksize=1,
                                       stride=1, act='silu'))
            self.cls_convs.append(nn.Sequential(*[
                DWConv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act='silu'),
                DWConv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act='silu')
            ]))
            self.cls_preds.append(nn.Conv2d(in_channels=int(256 * width), out_channels=num_classes, kernel_size=1,
                                            stride=1, padding=0))
            self.reg_convs.append(nn.Sequential(*[
                DWConv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act='silu'),
                DWConv(in_channels=int(256 * width), out_channels=int(256 * width), ksize=3, stride=1, act='silu')
            ]))
            self.reg_preds.append(nn.Conv2d(in_channels=int(256 * width), out_channels=4, kernel_size=1, stride=1,
                                            padding=0))
            self.obj_preds.append(nn.Conv2d(in_channels=int(256 * width), out_channels=1, kernel_size=1, stride=1,
                                            padding=0))
...
def parse_args():
    ...
    # 訓練權重資料位置，這裡一定要加載進去
    parser.add_argument('--pretrained', type=str, default=r'C:\DeepLearning\SpecialTopic\YoloxObjectDetection\check'
                                                          r'points\yolox_best_mAP.pth')
...
def main():
    ...
    model = YoloxObjectDetectionNano(num_classes=args.num_classes)
    ...
    with torch.no_grad():
        ...
        torch.onnx.export(model_script, images, 'YoloxObjectDetectionNano.onnx', input_names=input_names,
