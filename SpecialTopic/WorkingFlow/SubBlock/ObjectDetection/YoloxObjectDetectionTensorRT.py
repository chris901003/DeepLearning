...
import numpy as np
import cv2
from SpecialTopic.Deploy.YoloxObjectDetection.api import create_tensorrt_engine, tensorrt_engine_detect_image
...
class YoloxObjectDetectionTensorRT:
    def __init__(self, onnx_or_trt_file, classes_path, confidence, nms, filter_edge, cfg, fp16=True,
        ...
        """
        self.onnx_or_trt_file = onnx_or_trt_file
        if os.path.splitext(onnx_or_trt_file)[-1] == '.onnx':
            self.onnx_file = onnx_or_trt_file
            self.trt_file = None
        elif os.path.splitext(onnx_or_trt_file)[-1] == '.trt':
            self.onnx_file = None
            self.trt_file = onnx_or_trt_file
        else:
        ...
        # 初始化模型，同時會將訓練權重加入
        self.object_detection_model = create_tensorrt_engine(onnx_file_path=self.onnx_file, fp16_mode=fp16,
                                                             trt_engine_path=self.trt_file)
...
def test():
    import logging
    import time
    module = YoloxObjectDetectionTensorRT(onnx_or_trt_file='./checkpoint/object_detection/tensorrt/2022_11_7.trt',
                                          classes_path='./prepare/object_detection_classes.txt', confidence=0.7,
                                          nms=0.3, filter_edge=True, cfg='auto', new_track_box=10)
    ...
    while True:
        ...
        if ret:
            ...
            fps = 1 / (eTime - sTime)
            sTime = eTime
            info = f"fps: {fps}, wait: {num_wait_tracking_object}, tracking: {num_tracking_object}"
