...
net = net.to(device)
net.classifier[1] = nn.Linear(in_features=1280, out_features=200)
...
for epoch in range(1, 31):
    ...
    for idx, (images, labels) in enumerate(train_dataloader):
        ...
        exit()
    writer.add_scalar('Efficient train loss', total_loss / len(train_dataloader), epoch)
    writer.add_scalar('Efficient train accuracy', round(100 * (total_accuracy / total), 2), epoch)
    total_accuracy = 0
    net.eval()
    with torch.no_grad():
        ...
        writer.add_scalar('Efficient test accuracy', round(100 * (total_accuracy/len(test_dataset)), 2), epoch)
