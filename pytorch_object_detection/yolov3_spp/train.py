...
def train(hyp):
    ...
    # 解析一下my_data.data裡面有需要文件的路徑
    data_dict = parse_data_cfg(data)
    # 訓練照片的路徑，透過更改部分路徑可以獲取label的文件位置
    train_path = data_dict["train"]
    # 驗證的照片路徑，透過更改部分路徑可以獲取label的文件位置
    test_path = data_dict["valid"]
    nc = 1 if opt.single_cls else int(data_dict["classes"])  # number of classes
    # 一些超參數的調整，等到變成煉丹大師再來調整
    ...
    if opt.freeze_layers:
        # 只有yolo_layer以及predictor可以訓練，其他凍結
    ...
    # weight_decay也是一種防止過擬合的東西，可以讓學習的時候更加平滑
