...
class LossHistory():
    ...
    def __init__(self, log_dir, model, input_shape):
        ...
        try:
            # dummy_input先構建出由亂數組成的tensor且shape為(2, 3, input_shape[0], input_shape[1])
            dummy_input = torch.randn(2, 3, input_shape[0], input_shape[1])
            # writer構建出一個圖，這裡就是SummerWriter的應用了
...
class EvalCallback:
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        :param image_id: 圖檔名稱
        :param image: 圖像
        :param class_names: classes list
        :param map_out_path: 存擋位置
        :return:
        """
        ...
        #   也可以直接resize进行识别
        # ---------------------------------------------------------#
        # resize成輸入網路的圖像大小，resize前會先加上灰邊
        image_data = resize_image(image, (self.input_shape[1], self.input_shape[0]), self.letterbox_image)
        # ---------------------------------------------------------#
        #   添加上batch_size维度
        # ---------------------------------------------------------#
        # BGR -> RGB，同時添加batch_size
        ...
        with torch.no_grad():
            ...
            #   将预测框进行堆叠，然后进行非极大抑制
            # ---------------------------------------------------------#
            # result 7的内容为：x1, y1, x2, y2, obj_conf, class_conf, class_pred
