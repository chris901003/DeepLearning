...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        ...
        a, b, c, gxy, gwh, gij = extend_bbox(a, t)
        gi, gj = gij.T
...
def extend_bbox(a, t):
    ...
    gwh = t[:, 4:6]
    gij = gxy.long()
    case1_mask = (gxy[:, 0] % 1 < 0.5) & (gxy[:, 1] % 1 < 0.5)
    case2_mask = (gxy[:, 0] % 1 > 0.5) & (gxy[:, 1] % 1 < 0.5)
    case3_mask = (gxy[:, 0] % 1 > 0.5) & (gxy[:, 1] % 1 > 0.5)
    case4_mask = (gxy[:, 0] % 1 < 0.5) & (gxy[:, 1] % 1 > 0.5)
    res_gxy = torch.cat([gxy, gxy[case1_mask], gxy[case1_mask], gxy[case2_mask], gxy[case2_mask],
                     gxy[case3_mask], gxy[case3_mask], gxy[case4_mask], gxy[case4_mask]], dim=0)
    res_a = torch.cat([a, a[case1_mask], a[case1_mask], a[case2_mask], a[case2_mask],
                   a[case3_mask], a[case3_mask], a[case4_mask], a[case4_mask]], dim=0)
    res_b = torch.cat([b, b[case1_mask], b[case1_mask], b[case2_mask], b[case2_mask],
                   b[case3_mask], b[case3_mask], b[case4_mask], b[case4_mask]], dim=0)
    res_c = torch.cat([c, c[case1_mask], c[case1_mask], c[case2_mask], c[case2_mask],
                   c[case3_mask], c[case3_mask], c[case4_mask], c[case4_mask]], dim=0)
    res_gwh = torch.cat([gwh, gwh[case1_mask], gwh[case1_mask], gwh[case2_mask], gwh[case2_mask],
                     gwh[case3_mask], gwh[case3_mask], gwh[case4_mask], gwh[case4_mask]], dim=0)
    res_gij = torch.cat([gij,
                     gij[case1_mask] + torch.tensor([-1, 0]).to(device),
                     gij[case1_mask] + torch.tensor([0, -1]).to(device),
                     gij[case2_mask] + torch.tensor([1, 0]).to(device),
                     gij[case2_mask] + torch.tensor([0, -1]).to(device),
                     gij[case3_mask] + torch.tensor([1, 0]).to(device),
                     gij[case3_mask] + torch.tensor([0, 1]).to(device),
                     gij[case4_mask] + torch.tensor([-1, 0]).to(device),
                     gij[case4_mask] + torch.tensor([0, 1]).to(device)
                     ])
    return res_a, res_b, res_c, res_gxy, res_gwh, res_gij
